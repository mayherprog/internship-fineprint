...
updates = build.load_updates()
upd_out = out.parent / "updates.json"
upd_out.write_text(json.dumps(updates, ensure_ascii=False))
print(f"{out}: {len(rows)} programs; {upd_out}: {len(updates)} updates")
